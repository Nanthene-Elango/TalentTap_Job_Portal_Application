document.addEventListener('DOMContentLoaded', () => {
            ...
            function validateField(field) {
                ...
                switch(field.id) {
                    case 'fullNameInput':
                        return setFieldState(field, nameRegex.test(value) && value.length >= 2, 'Full name must be at least 2 characters (letters only)');
                    case 'usernameInput':
                        return setFieldState(field, usernameRegex.test(value) && value.length >= 3, 'Username must be at least 3 characters (letters and numbers only)');
                    case 'emailInput':
            ...
            const phoneInputs = [document.getElementById('phoneNumberInput'), document.getElementById('companyPhoneInput')];
            ...
            document.getElementById('confirmBtn').addEventListener('click', () => {
                ...
                if (password.value === "password123") { // Simulated correct password
                    ...
                    if (isEditingAccount) {
                        const accountCard = document.querySelector('#my-info .content-card:first-of-type');
                        const fieldValues = accountCard.querySelectorAll('.field-value');
                        const editFields = accountCard.querySelectorAll('.edit-field');
            ...
            document.getElementById('saveAccountBtn').addEventListener('click', () => {
                ...
                editFields.forEach(field => {
                    if (field.id !== 'phoneNumberInput' && field.id !== 'emailInput') { // Skip validation for readonly fields
                ...
                if (allValid) {
                    const fieldValues = accountCard.querySelectorAll('.field-value');
                    fieldValues.forEach((el, index) => {
                        if (el.id === 'phoneNumber') {
                            el.innerHTML = `${editFields[index].value}<span class="verified-badge" id="phoneVerified">Verified</span>`;
                        } else if (el.id === 'email') {
                            el.innerHTML = `${editFields[index].value}<span class="verified-badge" id="emailVerified">Verified</span>`;
                        } else {
                            el.innerHTML = editFields[index].value;
                        }
                    ...
                    document.getElementById('phoneVerified').style.display = 'inline-block';
